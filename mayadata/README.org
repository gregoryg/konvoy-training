...
Run the script =bin/kudo-re-init.sh= to assure KUDO is installed correctly on the cluster

#+begin_src sh :session konvoy-sh :shebang "#!/usr/bin/env bash" :results none :tangle bin/kudo-re-init.sh
...
kubectl kudo install kafka --instance=kafka \
		...
		-p BROKER_MEM=1024m \
		-p STORAGE_CLASS=openebs-hostpath \
		--version=0.1.3
#+end_src
