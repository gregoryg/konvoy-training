...
REGION=us-west-2
KEY_FILE=$(realpath *.pem | head -1) # path to private key file in folder where you ran konvoy -up
...
export CLUSTER=(cd state ; terraform output cluster_name) # name of your cluster, its the prefix used for worker nodes, check in ec2 console
...
#+begin_src sh :session konvoy-sh :results none
blkdevices=$(kubectl get blockdevices -n openebs|egrep -v '^NAME'|cut -d' ' -f1)
#+end_src
